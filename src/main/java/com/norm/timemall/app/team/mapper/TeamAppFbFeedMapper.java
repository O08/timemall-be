...
public interface TeamAppFbFeedMapper extends BaseMapper<AppFbFeed> {
    ...
    @Select("select b.avator authorAvatar, b.brand_name author,f.author_brand_id,f.comment_tag,f.title,f.preface,f.rich_media_content,f.cta_primary_label,f.cta_primary_url,f.cta_secondary_label,f.cta_secondary_url,f.cover_url,f.highlight,f.create_at,f.modified_at from app_fb_feed f inner join brand b on f.author_brand_id=b.id where f.id=#{id}")
