...
@Update("update oasis_join j set tag = #{tag} where id = #{id}")
    ...
    @Select("select o.handle,o.mark,o.avatar,o.title,o.id from oasis_join j inner join  oasis  o on j.oasis_id = o.id  inner join brand b2 on b2.id = o.initiator_id where j.brand_id=#{brand_id} and j.tag='2' and o.mark in( '2','4') order by j.modified_at desc")
    ...
    @Select("select o.handle,o.mark, o.avatar,o.title,o.id from oasis_join j inner join  oasis  o on j.oasis_id = o.id  inner join brand b2 on b2.id = o.initiator_id where j.brand_id=#{brand_id} and j.tag='2' and o.mark = '2' and ((o.for_private= '1' and  o.initiator_id=#{brand_id}) or o.for_private='0' ) order by j.modified_at desc")
